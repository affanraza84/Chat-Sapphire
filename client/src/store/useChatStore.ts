  ...
  persist(
    (set, get) => ({
      ...
      setSelectedUser: (selectedUser) => {
        // Unsubscribe from any previous user's socket events
        get().unsubscribeFromMessages();
        set({ selectedUser });
        if (selectedUser) {
          // Load messages for the newly selected user and subscribe
          get().getMessages(selectedUser._id);
          get().subscribeToMessages();
        }
      },

      // Called on app start to re‑hydrate state and fetch fresh messages if needed
      initialize: async () => {
        const { selectedUser } = get();
        if (selectedUser) {
    ...
    {
      name: "chat-store",
      // Persist only the selected user and message list (users list can be refetched)
      partialize: (state) => ({
